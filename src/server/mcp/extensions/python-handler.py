...
def call_handler(file_path, handler_type, params_path, result_path):
    ...
    try:
        ...
        # Check if this is a function call for dynamic extension
        if "function_name" in params:
            ...
            # Get the extension object (old style)
            extension = getattr(module, "extension", None)
            if extension is None:
                result = {"error": "No extension object found"}
            else:
                # Call the handler
                if handler_type == "tool":
                    result = extension.handler(params)
                elif handler_type == "resource":
                    uri = params["uri"]
                    uri_params = params["params"]
                    result = extension.handler(uri, uri_params)
                elif handler_type == "prompt":
                    result = extension.handler(params)
                else:
                    result = {"error": "Unknown handler type"}
