...
/**
 * Manages the webview panel for the AI Tools Dashboard
 */
export class SimpleWebviewProvider {
  ...
  private static getHtmlForWebview(webview: vscode.Webview, extensionUri: vscode.Uri): string {
    try {
      ...
      if (!fs.existsSync(cssFilePath) || !fs.existsSync(jsFilePath)) {
        return this.getErrorHtml(`Missing required assets. CSS: ${fs.existsSync(cssFilePath)}, JS: ${fs.existsSync(jsFilePath)}`);
      }

      // Create webview URIs for the assets
      const cssUri = webview.asWebviewUri(vscode.Uri.file(cssFilePath));
      const jsUri = webview.asWebviewUri(vscode.Uri.file(jsFilePath));
